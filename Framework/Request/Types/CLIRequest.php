...
class CLIRequest extends AbstractRequest implements ILogListener, IOptionProcessor {
     ...
     */
    function getShortOptions($fieldName, $shortName) {
     ...
     * @param \CPath\Framework\CLI\Option\Interfaces\IOptionMap $Map
     * @return void
     */
    function processMap(IOptionMap $Map) {
        foreach($this->mShortRequests as $short => $value) {
            try {
                $name = $Map->matchOption($short);
                $this->mRequest[$name] = $value;
            } catch (OptionMissingException $ex) {
